...
class Backbone(BackboneBase):
    ...
    def __init__(self,
        ...
        #在 DinoV2 模型被实例化之前，检查 out_feature_indexes。如果它的值是 [-1]，就将其替换为最后一层的实际索引。根据 DINOv2 的模型大小（base 或 large），最后一层的索引会有所不同
        if 'large' in name:
            num_layers = 24
        else:
            num_layers = 12
        ...
        self.encoder = DinoV2(
            size=name_parts[-1],
            out_feature_indexes=processed_out_feature_indexes,
            shape=target_shape,
