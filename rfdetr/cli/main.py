...
'''
v12
1.注释rfdetr\main.py的trainer函数与Roboflow工作区相关的无用代码
2.rfdetr\config.py的TrainConfig类dataset_file改为"coco"
3.rfdetr\config.py的TrainConfig类dataset_dir改为"../dataset"
4.创建dataset\train\_annotations.coco.json与dataset\train\example.json文件
.rfdetr\detr.py的train_from_config函数
'''
...
def trainer():
    ...
    parser.add_argument("--coco_dir", type=str, required=False, default="dataset")
